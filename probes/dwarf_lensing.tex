...
Note that some of our assumptions are clearly optimistic. In particular, the number density of the source we assumed is high, and the assumption of perfect lens selection is also unlikely to hold. Nevertheless, since the S/N ratio goes $\sim 1/\sqrt{N_\text{lens} N_\text{src}}$, lowering these numbers by a factor of $\sim 2$ would still yield a very high S/N ratio.
...
As mentioned earlier, a cored density profile is a signature of SIDM, hence we also calculate the shear signal for a modified NFW profile (with a core at the center,
...
We show its shear signal in \figref{dwarf_sn} to be compared with the signal from NFW profile. We see that the overall signal-to-noise does not change much with the profile.  
However, to statistically distinguish the different profiles, one needs to measure the shear at very small scales ($< 10$\,kpc), which is the regime where the systematics of the PSF dominate. In other words, the numbers of source and lens galaxies that LSST can find will be high enough to distinguish the difference between the two profiles provided ideal shape measurement. 
\YYM{more discussion on PSF?}
