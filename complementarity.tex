...
The LSST data set will uniquely complement several other experimental measurements to further improve our understanding of dark matter.
Below we summarize some of these complementary probes focusing on spectroscopic observations, high resolution imaging, indirect detection, and direct detection experiments.
Support of these experiements is essential to provide an comprehensive picture of dark matter physics.
The list here is not meant to be comprehensive, but rather serves to demonstrate the influence that LSST will have on dark matter studies generally.

% Spectroscopy
\subsection{Spectroscopy}
 \Contributors{Josh S., Ting L. ...}
\label{sec:spectroscopy}

\ADW{It would be good to have an overview here of the current and future telescopes/resources that would be necessary to pursue spectroscopy connected to LSST dark matter science. For example, Keck, Magellan, Gemini, MSE, ELTs, etc.}

In additional to the photometric and astrometric data from LSST, the dark matter constraints from various probes discussed in Section \ref{sec:probes} can be further improved with additional spectroscopic observations. In particular, spectroscopic follow-up studies will provide kinematic properties and redshifts of the objects found in LSST.
Given the faintness and high density targets that expected from LSST, community access to multi-object spectrograph on a large aperture telescope is required for these studies (cite the LSST-OIR report). Given that LSST is a imaging survey in the Southern Hemisphere, such spectroscopic facility is preferred in the south to maximize the overlap with LSST.

(TL: do we want to make a table to compare the specs of various spectroscopy facility. I do not think this is the scope of this paper...)

Many next generation telescopes and instruments are currently under preparation or construction, e.g., the Southern Spectroscopic Survey Instrument (SSSI), a project recommended for consideration by the DOE’s Cosmic Visions panel (arxiv.org/abs/1604.07626 and arxiv.org/abs/1604.07821), the Primary Focus Spectrograph (PFS) instrument on the Subaru telescope, the Maunakea Spectroscopic Explorer (MSE) at CFHT, a future ESO wide-field spectroscopic facility (need to check if this one has a name already or not), and many 30m class telescopes such as Thirty Meter Telescope (TMT), Giant Magellan Telescope (GMT), and European Extremely Large Telescope (E-ELT). 

In this section, we illustrate a few examples on how spectroscopy could improve the measurements on the dark matter properties for several probes.

\subsubsection{Milky Way Dwarfs \Contact{Josh}}
\Contributors{Josh, Ting, ...}
...
\subsubsection{Stellar Streams \Contact{Ting}}
\Contributors{Ting, ...}
...
\subsubsection{Galaxy Clusters \Contact{Will}}
\Contributors{Will, ...}
...
\subsubsection{Lyman-$\alpha$ Forest \Contact{Francis-Yan}}

\ADW{This is expected to be one paragraph from Francis-Yan.}

% High-resolution imaging
\subsection{High-Resolution Imaging \Contact{Will}}
\Contributors{Will, ...}
\label{sec:highres}
...
\subsubsection{Astrometric Microlensing of Compact Dark Matter \Contact{Will}}
\Contributors{Will, ...}
...
% Strong-microlensing
\subsubsection{Strong-Microlensing of Compact Dark Matter \Contributors{Will, ...}}
...
\subsubsection{Merging Galaxy Clusters and Cluster Subhalos \Contributors{Will D., Dave W., ...}}
...
\subsubsection{Strong Gravitational Lensing \Contributors{Chris F.} \label{sec:SLcomplement}}

All three approaches to use strong gravitational lens systems to make inferences on the nature of dark matter that were described in \S\ref{sec:stronglens}\ utilize LSST as a lens-finding facility.
...
\subsubsection{Baryon Scattering \Contact{Vera}}
...
\subsubsection{Local Dark Matter Velocity Distribution \Contact{Lina}}
\Contributors{Lina N.}
...
\begin{comment}
\subsection{Particle Accelerators \Contact{???}}
\Contributors{...}
\ADW{If no one steps up here, we should remove this section.}
\end{comment}

\begin{comment}
\subsection{Cosmic Microwave Background \Contributors{Vera,Kim,Francis-Yan,Cora}}
